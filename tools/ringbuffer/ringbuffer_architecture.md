...
- `ringbuffer.hpp`：对外公开接口与 `RingBuffer` 类定义。
- `ringbuffer.cpp`：核心实现。
- `ringbuffer_port.hpp`：可选的平台适配钩子，例如更强的临界区策略。
...
- `buffer_`：调用者提供的底层存储区指针。
- `capacity_`：逻辑容量，单位为字节。
- `head_`：下一个写位置对应的逻辑索引。
- `tail_`：下一个读位置对应的逻辑索引。
- `mask_`：容量为 2 的幂时用于快速回绕。
- `isPowerOfTwo_`：标记使用快速路径还是通用取模路径。
...
在 C++ 版本中，索引应使用 `std::atomic<uint32_t>` 明确表达 SPSC 下的可见性与发布顺序，而不是继续依赖 `volatile`。
